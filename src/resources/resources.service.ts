...
export class ResourcesService {
  ...
  async list(entityName: ResourceEntityName, query: ResourceQueryDto) {
    ...
    const filter = buildMongoFilter(parseFilter(query.filter));

    let cursor = model.find(filter).sort(buildMongoSort(query.sort)).skip(query.offset ?? 0);
  ...
  async create(entityName: ResourceEntityName, payload: LooseResourceDocument) {
    ...
    const record = {
      ...payload,
      id: typeof payload.id === 'string' ? payload.id : createEntityIdentifier(entityName),
      created_date:
        typeof payload.created_date === 'string' ? payload.created_date : timestamp,
  ...
  async bulkCreate(entityName: ResourceEntityName, items: LooseResourceDocument[]) {
    ...
    const records = items.map((item) => ({
      ...item,
      id: typeof item.id === 'string' ? item.id : createEntityIdentifier(entityName),
      created_date: typeof item.created_date === 'string' ? item.created_date : timestamp,
      updated_date: timestamp,
    }));
